...
public class OrdersController {
    ...
    public OrderController(OrderService orderService, ShoppingCartMapper mapper) {
    ...
    @PutMapping("{orderId}")
    public OrderResource updateOrder(@PathVariable Long orderId, @RequestBody UpdateShoppingCartResource resource){
        return mapper.toResource(orderService.update(orderId, mapper.toModel(resource)));
    }

    @DeleteMapping("{orderId}")
    public ResponseEntity<?> deleteOrder(@PathVariable Long ordertId){
